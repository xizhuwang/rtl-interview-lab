...
- The companion backpack contains the forge, equipped-state controls, resale, and every owned item. Each profession has a **weapon slot** and a **relic slot**, so two compatible items can be worn together. Unlocked professions use clean, unequipped base characters; selected gear is then composited onto the hands, back, or body instead of appearing as a detached corner badge. Every gear cell also has a generous transparent safety margin, so shop and backpack thumbnails never include pieces of neighboring items. Every purchase or equipment drop creates an independent copy with its own star level, so players can preserve a main build while enhancing spare copies. Each copy can be dragged onto the forge, equipped, enhanced, or sold for half its base price; the shop stays purchase-only and allows duplicate purchases. Each attempt consumes one Forge Hammer. Gear runs from 0 to 5 stars with success rates of 100%, 80%, 65%, 45%, and 30%. A failed attempt at 0–1 stars is protected; at 2 stars or above it drops one star. Equipment is never destroyed. Five-star Divine Gear adds a stronger visible aura and attack effect, but star level never changes scores, judging, hints, drops, or challenge difficulty.
- Daily check-in keeps its original 50-coin reward and seventh-day bonus. Separately, the deterministic daily RTL review records the current review streak, personal-best streak, and lifetime completion days without a fixed day limit. It grants 80 coins when passed, including when revisiting an already-solved exercise, and every seventh consecutive review raises the milestone bonus. Long-term milestones from 7 to 365 completed review days unlock playful titles and badges, while the compact panel keeps the detailed records collapsible. Review mode opens a temporary starter-code draft, never overwrites the saved answer, and hides previously unlocked hints, Golden behavior, and reasoning cards for solved exercises. All daily state stays in this browser.
- A contest-style three-problem RTL time trial provides an additional repeatable goal. Runs always restart from Starter code without overwriting saved solutions or exposing hints. Weekday practice uses a separate set and never grants coins or changes official records. Each weekend publishes a deterministic beginner/intermediate/advanced set; official scoring prioritizes solved count and then elapsed time, while the first, second, and third official solves grant one-time 200/350/600 coin rewards. Silver also grants one Forge Hammer and Gold grants two. Accumulated Gold weeks unlock a separate title-and-badge ladder from four to 52 weeks, and up to twelve prior weekly summaries remain on this browser.
- The first clear of an advanced or final BOSS has a 62% chance to drop 2–5 learning tools, a 15% chance to drop one equipment copy, and a 23% no-drop result. Equipment drops can be duplicates, giving players spare copies to enhance independently.
...
- Battle presentation uses separate profession attack poses—sword slash, bow release, healing cast, and circular timing magic—while respecting `prefers-reduced-motion` and leaving the judging path untouched.
