...
- The companion backpack contains the forge, equipped-state controls, resale, and every owned item. Owned equipment can be dragged onto the forge or upgraded from its own button; the shop stays purchase-only. Each attempt consumes one Forge Hammer. Gear runs from 0 to 5 stars with success rates of 100%, 80%, 65%, 45%, and 30%. A failed attempt at 0–1 stars is protected; at 2 stars or above it drops one star. Five-star Divine Gear adds a stronger visible aura and attack effect, but star level never changes scores, judging, hints, drops, or challenge difficulty. Selling gear resets its stars and refunds half of the current base price.
