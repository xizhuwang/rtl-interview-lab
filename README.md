...
Timing-sensitive exercises include bilingual golden behavior patterns that can be permanently unlocked per exercise with a Debug Visor. Compact logic briefs use a Timing Crystal, while each progressive hint uses one Chip Companion. Every new or migrated local profile receives 50 of each learning tool. After simulation, the browser runs the learner RTL and the reference RTL against the same stimulus, then aligns every DUT output as adjacent **Your / Golden** waveform rows. Shared inputs appear once, output badges identify matches or mismatches, and VCD aliases are preserved so a testbench wire cannot hide the corresponding DUT port. Golden waveforms are cached per exercise and rendering is capped to keep repeat runs responsive.
...
- Learning tools cost 100–160 points and equipment/enchantment upgrades stay within 100–500 points. A Debug Visor unlocks one exercise's Golden pattern, a Timing Crystal unlocks its compact logic brief, and a Chip Companion unlocks the next hint. Each unlock is saved locally and never consumes the same item twice.
